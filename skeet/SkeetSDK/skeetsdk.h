...
namespace SkeetSDK
{
	...
	bool ConfigSystem::LoadConfig(const char* name)
	{
		...
		if (cfgchunk.size())
		{
			*CurrentConfigHash = Hasher::FNV1a(name);
			...
			for (auto& cfg : Menu->Tabs[CONFIG]->CEs[3]->GetAs<Listbox>()->Info.Items)
			{
	...
	void Renderer::InitFinal()
	{
		if (FinalRenderHook != nullptr) return;
		FinalRenderHook = Memory::DetourHook::Hook(
			Memory::CheatChunk.find("53 56 57 8B F9 8B 57 08 8D 82 A8 00 00 00 F7 DA 1B D2 23 D0 52"),
			RenderFinalListener);
	};
